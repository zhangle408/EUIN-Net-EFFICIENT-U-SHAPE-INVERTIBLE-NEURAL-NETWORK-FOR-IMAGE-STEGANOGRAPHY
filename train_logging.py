...
util.setup_logger('train', '/data/EUIN-Net/newlogging', 'train_', level=logging.INFO, screen=True, tofile=True)
...
try:
    ...
    for i_epoch in range(c.epochs):
        # if i_epoch > 30:
        #     adjust_learning_rate(optim, i_epoch)
        ...
        for i_batch, datalist in enumerate(datasets.trainloader):
            ...
            data = data.to(device)
            secret = data[data.shape[0] // 3:]  # data
            cover = data[:data.shape[0] // 3]  # data
            secret = secret.view(secret.shape[0] // 2, secret.shape[1] * 2, secret.shape[2], secret.shape[3])
            ...
            if i_batch == c.iters_per_epoch-1:
              break
        ...
        if i_epoch % c.val_freq == 0:
            with torch.no_grad():
                ...
                psnr_c = []
                net.eval()
                for i_batch, xlist in enumerate(datasets.testloader):
                    x,labels = xlist
                    x = x.to(device)
                    secret = data[data.shape[0] // 3:]  # data
                    cover = data[:data.shape[0] // 3]  # data
