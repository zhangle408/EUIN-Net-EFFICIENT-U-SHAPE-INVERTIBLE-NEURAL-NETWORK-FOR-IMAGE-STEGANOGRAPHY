...
with torch.no_grad():
    ...
    import time
    start = time.time()
    for i, xx in enumerate(datasets.trainloader):
        data, lable = xx
        data = data.to(device)
        ...
        psnr_c = np.zeros(N)
        cover_img_numpy = bgr2ycbcr(cover_img_numpy)
        container_img_numpy = bgr2ycbcr(container_img_numpy)
        for i in range(N):
        ...
        psnr_s = np.zeros(N)
        secret_img_numpy1 = bgr2ycbcr(secret_img_numpy[:, :, :,:3])
        rev_secret_numpy1 = bgr2ycbcr(rev_secret_numpy[:, :, :,:3])

        secret_img_numpy2 = bgr2ycbcr(secret_img_numpy[:, :, :, 3:])
        rev_secret_numpy2 = bgr2ycbcr(rev_secret_numpy[:, :, :, 3:])
        secret_img_numpy = np.concatenate(
            (secret_img_numpy1.reshape(16, 128, 128, 1), secret_img_numpy2.reshape(16, 128, 128, 1)), 3)
        rev_secret_numpy = np.concatenate(
            (rev_secret_numpy1.reshape(16, 128, 128, 1), rev_secret_numpy2.reshape(16, 128, 128, 1)), 3)
